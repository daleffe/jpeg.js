var JPEG = (function (module) {
    ...
    module.Stream = function (args) {
        ...
        function getFrame(isSnapshot = false) {
                    ...
                    try {
                        if (self.request.status == 200) {
                            ...
                            if (self.contentType.startsWith('multipart/x-mixed-replace;') && self.request.readyState == self.request.LOADING) {
                                console.warn("Check out mjpeg.js (https://github.com/daleffe/mjpeg.js) to read MJPEG streams!");
                        ...
                        if (self.request.readyState == self.request.DONE) {
                            ...
                            if (self.contentType.length > 0) {
                                if (self.contentType.startsWith('multipart/x-mixed-replace;')) {
                                    self.onError(JSON.stringify({ status: -2147483648, message: "Check out mjpeg.js (https://github.com/daleffe/mjpeg.js) to read MJPEG streams!" }));
                                    return;
                                }
            ...
            try {
                self.request.open("GET", self.url.href, true);
    ...
    module.Player = function (container, url, username, password, options) {
        console.info("Check 'jpegstream.js' for old-alternative fetch version");
